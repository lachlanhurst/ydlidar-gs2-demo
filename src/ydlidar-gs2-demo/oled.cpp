...
// The main changes are a switch from the bit-bashing
// approach to using the arduino Wire library
...
OLED::OLED(tWidth width, tHeight height, tDisplayCtrl displayCtrl, uint8_t i2c_address) :
width(width),
height(height),
i2c_address(i2c_address),
...
void OLED::begin(TwoWire &I2Cpipe)
{
    wire = &I2Cpipe;

    wire->beginTransmission( i2c_address );
    // wire->write(i2c_address << 1); // address + write
    wire->write(0x00); // command
    wire->write(0xAE); // display off
    wire->write(0xD5); // clock divider
    wire->write(0x80); 
    wire->write(0xA8); // multiplex ratio
    wire->write(height - 1); 
    wire->write(0xD3); // no display offset
    wire->write(0x00); 
    wire->write(0x40); // start line address=0
    wire->write(0x8D); // enable charge pump
    wire->write(0x14); 
    wire->write(0x20); // memory adressing mode=horizontal
    wire->write(0x00); 
    wire->write(0xA1); // segment remapping mode
    wire->write(0xC8); // COM output scan direction
    if (width == 128 && height == 32)
    {
        wire->write(0xDA); // com pins hardware configuration
        wire->write(0x02); 
    }
...
void OLED::set_power(bool enable)
{
    ...
    {
        wire->write(0xAE); // display off
        wire->write(0x8D); // disable charge pump
        wire->write(0x10);
    }
    wire->endTransmission();
}
...
void OLED::set_invert(bool enable)
{
    ...
    if (enable)
    {
        wire->write(0xA7); // invert display
    }
    else
    {
...
void OLED::set_scrolling(tScrollEffect scroll_type, uint_fast8_t first_page, uint_fast8_t last_page)
{
    ...
    // wire->write(i2c_address << 1); // address + write
    wire->write(0x00); // command
    wire->write(0x2E); // deativate scroll
    if (scroll_type == DIAGONAL_LEFT || scroll_type == DIAGONAL_RIGHT)
    {
        wire->write(0xA3); // vertical scroll area
        wire->write(0x00);
        wire->write(height - 1);
    }
    if (scroll_type != NO_SCROLLING)
    {
        wire->write(scroll_type);
        wire->write(0x00); // dummy byte
        wire->write(first_page);
        wire->write(0x00); // time interval
        wire->write(last_page);
        if (scroll_type == DIAGONAL_LEFT || scroll_type == DIAGONAL_RIGHT)
        {
            wire->write(0x01); // vertical scrolling speed
        }
        else
        {
            wire->write(0x00); // dummy byte
        ...
        wire->write(0x2F); // ativate scroll
    }
    wire->endTransmission();
}
...
void OLED::set_contrast(uint8_t contrast)
{
    ...
    // wire->write(i2c_address << 1); // address + write
    wire->write(0x00); // command
    wire->write(0x81); // deativate scrol
    wire->write(contrast); 
    wire->endTransmission();
}
...
void OLED::display()
{
  ...
  wire->beginTransmission(i2c_address);
  wire->write(0x00); // command
  wire->write(0xB0); // set page
  wire->write(0x21); // column address
  wire->write(0x00); // first column =0
  wire->write(width - 1); // last column
  wire->endTransmission();
